	    ...
	    struct stream * s, struct ospf_interface *oi, int size)
{
  ...
  if (!CHECK_FLAG (hello->options, OSPF_OPTION_MT)
      && (oi->area->default_exclusion == OSPF_DEFAULT_EXCLUSION_ENABLE))
    {
      ...
      zlog_warn ("Packet %s [Hello:RECV]: MT-bit off. This area has default exclusion enabled, drop it.",
		 inet_ntoa (ospfh->router_id));
      return;
    }
	      ...
	      struct stream *s, struct ospf_interface *oi, u_int16_t size)
{
  ...
  if (!CHECK_FLAG (dd->options, OSPF_OPTION_MT)
      && (oi->area->default_exclusion == OSPF_DEFAULT_EXCLUSION_ENABLE))
    {
       ...
       * these conditions
       */
      zlog_warn ("Packet[DD]: Neighbor %s: has MT-bit off and this area has default exclusion enabled",
		 inet_ntoa (nbr->router_id));
      return;
    }
