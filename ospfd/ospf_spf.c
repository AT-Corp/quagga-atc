  ...
  if (w->type == OSPF_ROUTER_LSA)
    {
      ...
      /* XXX length is 20 - lsa hdr, 4 - non-link info in lsa body */
...
ospf_nexthop_calculation (struct ospf_area *area, u_int8_t mt_id, 
                          ...
                          struct router_lsa_link *l, unsigned int distance, int lsa_pos)
{
  struct listnode *node, *nnode;
	       ...
	       u_int8_t mt_id, struct pqueue * candidate)
{
  ...
  while (p < lim)
      ...
      if (w_lsa->stat == LSA_SPF_NOT_EXPLORED)
	{
          ...
          if (ospf_nexthop_calculation (area, mt_id, v, w, l, distance, lsa_pos))
      ...
      else if (w_lsa->stat >= 0)
	{
	  ...
	  if (w->distance < distance)
	      ...
	      ospf_nexthop_calculation (area, mt_id, v, w, l, distance, lsa_pos);
            }
           /* less than. */
	  else
            {
  ...
  if (v->type == OSPF_VERTEX_ROUTER)
    {
      ...
      while (p < lim)
	    ...
	    ospf_intra_add_stub (rt, l, v, area, mt_id, parent_is_root, lsa_pos);
	  lsa_pos++;
        }
    }
